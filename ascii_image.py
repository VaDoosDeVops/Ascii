    ...
    for i in ascii_pixels:
        if i != '\n':
            if _256_color:
                ...
                print(colorama.Fore.WHITE + colorama.Back.BLACK + i, end='')
                idx += 1
        else:
            print(i, end='')
    print('')
...
def main():
    ...
    if args.url:
        ...
        parser.error(
            "Please specify either a file path or a URL to convert to ASCII.")

    if args.json:
        export_json(args.json, img, args.resolution)
    else:
        print_image(img, args.resolution, greyscale=not args.color,
                    bg_color=args.background, fg_color=args.color)
